...
public class WeaponFactory {
    ...
    private Color getColor(Weapon.WeaponName weaponName) {
    ...
    private AmmoPack getFullCost(Weapon.WeaponName weaponName) {
        switch (weaponName) {
    ...
    private AmmoPack getReducedCost(Weapon.WeaponName weaponName) {
    ...
    private List<FireMode.FireModeName> getNameList(Weapon.WeaponName weaponName) {
    ...
    private TargetFinder getTargetFinder(Weapon.WeaponName weaponName, FireMode.FireModeName fireModeName) {
        ...
        targetMap.put(OPTION1, p -> (p.getMainTargets().isEmpty()? new ArrayList<>():board.getVisible(p.getPosition()).stream()
                .map(Square::getPlayers)
                .flatMap(x -> x.stream())
                .distinct()
    ...
    private DestinationFinder getDestinationFinder(Weapon.WeaponName weaponName, FireMode.FireModeName fireModeName) {
        ...
        destinationMap = new EnumMap<>(FireMode.FireModeName.class);
        destinationMap.put(MAIN, (p, t) -> new ArrayList<>());
        destinationMap.put(OPTION1, (p, t) -> {
            List<Square > l = board.getReachable(p.getPosition(), 2);
        ...
        destinationMap.put(OPTION1, (p, t) -> p.getMainTargets().isEmpty() ? new ArrayList<>() : Arrays.asList(p.getMainTargets().get(0).getPosition()));
        ...
        destinationMap = new EnumMap<>(FireMode.FireModeName.class);
        destinationMap.put(MAIN, (p, t) -> t.isEmpty() ? new ArrayList<>() : board.getReachable(t.get(0).getPosition(), 1).stream().filter(x->!x.containsPlayer(t.get(0))).collect(Collectors.toList()));
        destinationMap.put(OPTION1, (p, t) -> new ArrayList<>());
        ...
        destinationMap.put(OPTION1, (p, t) -> {
            if (p.getMainTargets().isEmpty()) {
                return board.getReachable(p.getPosition(), 1).stream().filter(x -> x.getPlayers().size() > 1 || !x.getPlayers().contains(p)).collect(Collectors.toList());
            }
    ...
    private Integer getTargetNumber (Weapon.WeaponName weaponName, FireMode.FireModeName fireModeName) {
        Map<Weapon.WeaponName, Map> fireModeMap = new EnumMap<>(Weapon.WeaponName.class);
    ...
    private AmmoPack getFireModeCost (Weapon.WeaponName weaponName, FireMode.FireModeName fireModeName) {
