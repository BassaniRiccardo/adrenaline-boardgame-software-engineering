...
public class GameEngine implements Runnable{
    ...
    private void configurePlayers(){
        ...
        for (PlayerController p : players) {
            ...
            board.getPlayers().add(p.getPlayer());
    ...
    public void resolve(){
        ...
        Collections.sort(players, (p1,p2) -> {
            if (p1.getPlayer().getPoints() > p2.getPlayer().getPoints()) return -1;
            else if (p1.getPlayer().getPoints() < p2.getPlayer().getPoints()) return 1;
            else {
        ...
        if (players.get(0).getPlayer().getPoints() == players.get(1).getPlayer().getPoints() && !killShotTrack.getKillers().contains(players.get(0).getPlayer()) && !killShotTrack.getKillers().contains(players.get(1).getPlayer())) {
            System.out.println("Player " + players.get(0).getPlayer().getId() + " and Player " + players.get(1).getPlayer().getId() + ", you did not kill anyone. Shame on you! The game ends with a draw.\n");
            for (int i = 2; i < players.size(); i++) {
                System.out.println("Player " + players.get(i).getPlayer().getId() + ", " + players.get(i).getPlayer().getPoints() + " points.");
            }
        } else {
            System.out.println("Winner: Player " + players.get(0).getPlayer().getId() + ", with " + players.get(0).getPlayer().getPoints() + " points!\n");
        ...
        if (!frenzy) gameOver = true;
            ...
            for (Player p : board.getPlayers()){
                if (p.getDamages().isEmpty()){
                    p.setFlipped(true);
                    p.setPointsToGive(2);
                    int frenzyActivator = currentPlayer.getPlayer().getId();
